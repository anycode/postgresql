...
int? secsSince(DateTime? time) =>
    time == null ? null : new DateTime.now().difference(time).inSeconds;

debug(Pool pool) {
  int total = pool.connections.length;

  int available = pool.connections
      .where((c) => c.state == PooledConnectionState.available)
      .length;

  int inUse = pool.connections
      .where((c) => c.state == PooledConnectionState.inUse)
      .length;
  ...
  int testing = pool.connections
      .where((c) => c.state == PooledConnectionState.testing)
      .length;

  int connecting = pool.connections
      .where((c) => c.state == PooledConnectionState.connecting)
      .length;

  print(
      'pool: ${pool.state} total: $total  available: $available  in-use: $inUse  testing: $testing connecting: $connecting  waiting: $waiting leaked: $leaked');
  pool.connections.forEach((c) => print(
      '${c.name} ${c.state} ${c.connectionState}  est: ${secsSince(c.established)}  obt: ${secsSince(c.obtained)}  rls: ${secsSince(c.released)}  leaked: ${c.isLeaked}'));
  print('');
}
...
main() {
  ...
  var loggerFunc = (Timer? t) {
    print(
        'queriesSent: $queriesSent  queriesCompleted: $queriesCompleted  slowSent: $slowQueriesSent  slowCompleted: $slowQueriesCompleted  connect timeouts: $connectTimeout  queryError: $queryError   connectError: $connectError ');
    debug(pool);
  };
...
//    var pass = expectAsync(() {});
  ...
  testConnect(_) {
    pool.connect().then((conn) {
      ...
      queriesSent++;
      conn.query("select 'oi';").toList().then((rows) {
        queriesCompleted++;
        expect(rows[0].toList(), equals(['oi']));
      }).catchError((err) {
        _log('Query error: $err');
        queryError++;
      }).whenComplete(() {
  ...
  slowQuery() {
    pool.connect().then((conn) {
      ...
      slowQueriesSent++;
      conn.query("select generate_series (1, 100000);").toList().then((rows) {
  ...
  pool.start().then((_) {
    timer = new Timer.periodic(queryPeriod, (_) {
      for (var i = 0; i < slowQueries; i++) slowQuery();
      for (var i = 0; i < testConnects; i++) testConnect(null);
    });
  }).catchError((err, st) {
