...
class ParameterStatus implements ProtocolMessage {
  ...
  @override
  List<int> encode() {
    var mb = new _MessageBuilder(messageCode)..addUtf8(name)..addUtf8(value);
    return mb.build();
  }
...
class Query implements ProtocolMessage {
  ...
  @override
  List<int> encode() => (new _MessageBuilder(messageCode)..addUtf8(query))
      .build(); //FIXME why do I need extra parens here. Analyzer bug?
...
class RowDescription implements ProtocolMessage {
  ...
  @override
  List<int> encode() {
    var mb = new _MessageBuilder(messageCode)..addInt16(fields.length);
...
class DataRow implements ProtocolMessage {
  ...
  @override
  List<int> encode() {
    var mb = new _MessageBuilder(messageCode)..addInt16(values.length);
  ...
  String toString() => json.encode({
        ...
        'code': new String.fromCharCode(messageCode),
        'values': values
            .map(utf8.decode) //TODO not all DataRows are text, some are binary.
      });
}
...
class CommandComplete implements ProtocolMessage {
  ...
  @override
  List<int> encode() => (new _MessageBuilder(messageCode)..addUtf8(tag))
      .build(); //FIXME remove extra parens.
