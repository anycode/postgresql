...
class PoolImpl implements Pool {
  ...
  final _messages = StreamController<pg.Message>.broadcast();
  ...
  Future _establishConnectionSafely() async {
    for (DateTime? since;;) //#21: try a while since PG likely recovering
      try {
        ...
        if (since == null) {
          since = now;
          _messages.add(pg.ClientMessage(
              severity: 'WARNING',
  ...
  Future<PooledConnectionImpl> _connect(Duration timeout) async {
    ...
    // wait queue.
    if (pconn == null) {
      final waiting = _Waiting(settings.limitConnections > 0
  ...
  void _releaseConnection(PooledConnectionImpl pconn) {
    ...
    } else if (_isExpired(pconn._established!,
        settings.maxLifetime + pconn._extraLifetime)) {
      _destroyConnection(pconn);
      _establishConnectionSafely()
      .then(_processWaitQueue);
  ...
  @override
  Future stop() {
    if (state == stopped || state == initial) return Future.value();
...
class _Waiting {
  ...
  _Waiting(bool runOut): c = Completer<PooledConnectionImpl>() {
